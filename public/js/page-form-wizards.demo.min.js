/*   
Template Name: Source Admin - Responsive Admin Dashboard Template build with Twitter Bootstrap 3.3.7 & Bootstrap 4
Version: 1.4.0
Author: Sean Ngu
Website: http://www.seantheme.com/source-admin-v1.4/admin/
*/var handleBootstrapWizardsValidation=function(){"use strict";$("#wizard").bwizard({validating:function(e,a){return(0==a.index&&a.nextIndex>=0||a.nextIndex>0)&&!1===$('form[name="form-wizard"]').parsley().validate("wizard-step-1")?!1:(1==a.index&&a.nextIndex>=1||a.nextIndex>1)&&!1===$('form[name="form-wizard"]').parsley().validate("wizard-step-2")?!1:(2==a.index&&a.nextIndex>=2||a.nextIndex>2)&&!1===$('form[name="form-wizard"]').parsley().validate("wizard-step-3")?!1:(3==a.index&&a.nextIndex>=3||a.nextIndex>3)&&!1===$('form[name="form-wizard"]').parsley().validate("wizard-step-4")?!1:void 0}})},PageDemo=function(){"use strict";return{init:function(){handleBootstrapWizardsValidation()}}}();